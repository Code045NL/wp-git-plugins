...
class WP_Git_Plugins_Repository {
    ...
    private $debug_log = [];
    private $debug_log_option = 'wp_git_plugins_debug_log';
    /**
    ...
    public function clear_debug_log() {
        $this->debug_log = [];
        update_option($this->debug_log_option, $this->debug_log);
        return true;
    }
    private function log($message, $data = null) {
        ...
        if (count($this->debug_log) > 200) {
            $this->debug_log = array_slice($this->debug_log, -200);
        }
        update_option($this->debug_log_option, $this->debug_log);
    ...
    public function __construct() {
        ...
        $this->debug_log = get_option($this->debug_log_option, []);
    }
    public function add_repository($repo_url, $branch = 'main') {
        global $wpdb;
        $table = $wpdb->prefix . 'wpgp_repos';
        // Check if repository already exists
        $exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table WHERE repo_url = %s", $repo_url));
        if ($exists) {
        ...
        $this->log('Repository added', $repo_url);
    ...
    public function get_repositories() {
        global $wpdb;
        $table = $wpdb->prefix . 'wpgp_repos';
        $rows = $wpdb->get_results("SELECT * FROM $table ORDER BY added_at DESC");
        $repos = [];
        foreach ($rows as $row) {
            $repos[] = [
                'url' => $row->repo_url,
    ...
    public function get_repository($url) {
        global $wpdb;
        $table = $wpdb->prefix . 'wpgp_repos';
        $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE repo_url = %s", $url));
        if ($row) {
            return [
                'url' => $row->repo_url,
                'branch' => $row->branch,
    ...
    public function update_repository($url, $data) {
        global $wpdb;
        $table = $wpdb->prefix . 'wpgp_repos';
        $updated = $wpdb->update($table, $data, ['repo_url' => $url]);
        if ($updated) {
            $this->log('Repository updated', $url);
            return true;
        }
        return new WP_Error('repo_not_found', __('Repository not found.', 'wp-git-plugins'));
    }
    
    public function remove_repository($repo_url) {
        global $wpdb;
        $table = $wpdb->prefix . 'wpgp_repos';
        $deleted = $wpdb->delete($table, ['repo_url' => $repo_url]);
        if ($deleted) {
            $this->log('Repository removed', $repo_url);
            return true;
        }
        return new WP_Error('repo_not_found', __('Repository not found.', 'wp-git-plugins'));
    }
    ...
    public function change_repository_branch($repo_url, $branch, $remove_and_clone = 0) {
        $repositories = $this->get_repositories();
        $found = false;
        foreach ($repositories as &$repo) {
        ...
        if ($remove_and_clone) {
            ...
            $cmd = sprintf('git clone --branch %s --single-branch --depth 1 %s %s 2>&1',
                escapeshellarg($branch),
                escapeshellarg($clone_url),
