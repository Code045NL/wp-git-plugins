...
class WP_Git_Plugins_Admin {
    ...
    public function enqueue_scripts($hook) {
        ...
        // Removed global JS injection for GitHub credentials (private repo support removed)
    }
    ...
    public function register_hooks() {
        ...
        add_action('wp_ajax_wp_git_plugins_add_repo', [$this, 'ajax_add_repo']);
        // Enqueue admin styles and scripts
        add_action('admin_enqueue_scripts', [$this, 'enqueue_styles']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_scripts']);
        // Inject global JS variables for GitHub user
        add_action('admin_head', [$this, 'inject_github_user_js']);
    ...
    public function ajax_check_updates() {
        ...
        /**
     * AJAX handler for adding a repository
     */
    public function ajax_add_repo() {
        check_ajax_referer('wp_git_plugins_ajax', 'nonce');
        if (!current_user_can('install_plugins')) {
            wp_send_json_error(['message' => __('You do not have sufficient permissions to add repositories.', 'wp-git-plugins')]);
        }
        $repo_url = isset($_POST['repo_url']) ? esc_url_raw($_POST['repo_url']) : '';
        ...
        if (empty($repo_url)) {
            wp_send_json_error(['message' => __('Repository URL is required.', 'wp-git-plugins')]);
        }
        $repository = new WP_Git_Plugins_Repository();
        $result = $repository->add_repository($repo_url, $is_private, $branch);
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()]);
        }
        wp_send_json_success(['message' => __('Repository added successfully.', 'wp-git-plugins')]);
    }
    ...
    public function ajax_get_branches() {
        ...
        /**
     * AJAX handler for changing repository branch and optionally removing/cloning
     */
    public function ajax_change_branch() {
        check_ajax_referer('wp_git_plugins_ajax', 'nonce');
        if (!current_user_can('install_plugins')) {
            wp_send_json_error(['message' => __('You do not have sufficient permissions to change branches.', 'wp-git-plugins')]);
        }
        $repo_url = isset($_POST['repo_url']) ? esc_url_raw($_POST['repo_url']) : '';
        $branch = isset($_POST['branch']) ? sanitize_text_field($_POST['branch']) : '';
        $remove_and_clone = isset($_POST['remove_and_clone']) ? intval($_POST['remove_and_clone']) : 0;
        if (empty($repo_url) || empty($branch)) {
            wp_send_json_error(['message' => __('Repository URL and branch are required.', 'wp-git-plugins')]);
        }
        $repository = new WP_Git_Plugins_Repository();
        $result = $repository->change_repository_branch($repo_url, $branch, $remove_and_clone);
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()]);
        }
    ...
    public function inject_github_user_js() {
        $options = get_option('wp_git_plugins_options', []);
        ?>
        <script>
        window.wpGitPluginsUser = {
            githubUsername: <?php echo json_encode($options['github_username'] ?? ''); ?>,
            githubToken: <?php echo json_encode($options['github_access_token'] ?? ''); ?>
        };
        </script>
        <?php
    }
